...
- Diet Platform:  /opt/diet_platform      (port 8150) — «Пухляш»: сбор и выдача диет
...
═══ ПАМЯТЬ И ДОСТАВКА ═══
- В НАЧАЛЕ каждой задачи: kb_search по теме задачи
- В КОНЦЕ каждой задачи: kb_save с резюме и списком файлов
- Если в запросе слова "отчёт/файл/архив/скачать/пришли": 
  pack_to_zip → send_file_to_tg (не текстом в чат)

═══ DIET PLATFORM (ПУХЛЯШ) ═══
Управляешь Diet Platform через diet_* инструменты:
1. diet_health  — проверить сервис
2. diet_search  — запустить поиск (session_id)
3. diet_status  — статус сессии
4. diet_list    — список диет
5. diet_pending — диеты на модерации
6. diet_verify  — одобрить/отклонить диету
7. diet_dlq     — ошибки конвейера
Diet Platform агентом не управляет. Зависимость односторонняя.
...
class TaskStatus(str, Enum):
    ...
    FAILED    = "failed"
    CANCELLED = "cancelled"
    PAUSED    = "paused"     # 429 backoff — hot-resume после восстановления API
...
class Task:
    ...
    # ── Phase 1: PAUSED state + fire-and-forget ──────────────────
    paused_at:       float = 0.0    # timestamp паузы (0 = не пауза)
    pause_reason:    str   = ""     # причина паузы
    current_step:    int   = 0      # последний выполненный шаг (для hot-resume)
    fire_and_forget: bool  = False  # молчание в TG до финала
...
class LeviathanAgent:
    ...
    def __init__(
        ...
        self._storage_ref       = None   # заполняется AgentRunner для backoff save
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                ...
                if "429" in err_str or "quota" in err_str.lower():
                    ...
                    # ── Phase 1: если все ключи заблокированы ─ backoff ──
                    if hasattr(self.key_pool, "all_rate_limited") and self.key_pool.all_rate_limited():
                        plog(task.id, PipelineEvent.CAUGHT_429,
                             f"iter={iteration} key=...{key[-6:]} all_keys_blocked")
                        from execution.backoff_scheduler import BackoffScheduler
                        scheduler = BackoffScheduler(self.key_pool)
                        await scheduler.pause_and_wait(task, storage=self._storage_ref)
                        plog(task.id, PipelineEvent.TASK_RESUMED, f"шаг={len(task.steps)}")
                        task.current_step = len(task.steps)
                    else:
                        plog(task.id, PipelineEvent.CAUGHT_429,
                             f"iter={iteration} key=...{key[-6:]} retry_next")
                    continue
            ...
            for part in parts:
                if hasattr(part, "text") and part.text:
                    ...
                    if len(parts) == 1:
                        ...
                        task.finished_at = time.time()
                        plog(task.id, PipelineEvent.TASK_COMPLETE,
                             f"шагов={len(task.steps)} iter={iteration}")
                        await self._close_journal(task)
                        return task
    ...
    async def _execute_tool(
        ...
        if self.on_step and not task.fire_and_forget:
...
try:
    from agent.intent import build_intent_prefix as _build_intent_prefix
    _INTENT_AVAILABLE = True
except ImportError:
    _INTENT_AVAILABLE = False
    def _build_intent_prefix(text): return ""
